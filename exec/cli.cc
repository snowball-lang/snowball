...
#define CHECK_ARG(expectation)                                                 \
    if (current_index == args.size() - 1) {                                    \
        throw SNError(Error::ARGUMENT_ERROR,                                   \
                      "Expected " expectation " as argument");                 \
    }
    ...
    if (current_arg == "help") {
        if (current_index < args.size() - 1) {
            ...
            if (IF_ARG("build")) {
                ...
                Logger::log("Options:");
                Logger::log(
                    "  --emit_type {type}        - Select the output format");
                Logger::log(
                    "  --test [-t]               - Build project as it was for "
                    "testing");
                Logger::log(
                    "  --optimization [-o] {opt} - Optimize your project at a "
                    "certain level");
                Logger::log("");
                Logger::log("Emit types:");
                Logger::log(
                    "  exec                      - Build your project as an "
                    "executable");
                Logger::log(
                    "  lib                       - Build a shared library from "
                    "your project");
                Logger::log(
                    "  llvm-ir                   - Emit the llvm-ir code to "
                    "file");
                Logger::log("");
                ...
                Logger::log(
                    "  O1 (default)              - Optimize quickly without "
                    "destroying debuggability");
                Logger::log(
                    "  O2                        - Like O3 without triggering "
                    "significant incremental compile time or code size growth");
                Logger::log(
                    "  O3                        - Optimize for fast execution "
                    "as much as possible");
                Logger::log(
                    "  Os                        - Similar to O2 but tries to "
                    "optimize for small code size instead of fast execution");
                Logger::log(
                    "  Oz                        - A very specialized mode "
                    "that will optimize for code size at any and all costs");
            } else if (IF_ARG("run")) {
                Logger::log("Snowball (C) MIT");
                ...
                Logger::info(
                    "You can get your tests as an executable by executing:\n   "
                    "-> `snowball build test`");
            } else if (IF_ARG("init")) {
                Logger::log("Snowball (C) MIT");
                ...
                Logger::log("Options:");
                Logger::log(
                    "  --yes (-y)        - Stop asking me you stupid bot!");
                Logger::log(
                    "  --lib (-l)        - Create a project designed as a "
                    "library");
                Logger::log(
                    "  --cfg (-c)        - Create only a configuration file");
                Logger::log(
                    "  --skip-cfg        - Create the project without a config "
                    "file (errors if `--cfg` is passed)");
                Logger::log("");
                Logger::info("You can create a new folder by executing:\n   -> "
                             "`snowball new {name}`");
            } else {
                throw SNError(ARGUMENT_ERROR, FMT("Command '%s' not found.",
                                                  current_arg.c_str()));
            }
        } else {
            help();
            ...
            if (IF_ANY_ARG("--test", "-t")) {
                ...
                if (current_arg == "exec") {
                    ...
                    throw SNError(
                        ...
                        "Valid build output types are: exec, lib, llvm-ir");
                }
                ...
                throw SNError(
                    Error::ARGUMENT_ERROR,
                    FMT("Unexpected argument for the build command: %s",
                        current_arg.c_str()));
            }
        }
        ...
        while (current_index < args.size() - 1) {
            ...
            if (IF_ANY_ARG("--silent", "-s")) {
                ...
                throw SNError(
                    Error::ARGUMENT_ERROR,
                    FMT("Unexpected argument for the test command: %s",
                        current_arg.c_str()));
            }
        }
        ...
        while (current_index < args.size() - 1) {
            ...
            if (IF_ANY_ARG("--yes", "-y")) {
                ...
                throw SNError(
                    Error::ARGUMENT_ERROR,
                    FMT("Unexpected argument for the init command: %s",
                        current_arg.c_str()));
            }
        }
    } else {
        throw SNError(Error::ARGUMENT_ERROR,
                      FMT("Unknown command found: %s", current_arg.c_str()));
    }
...
void CLI::help() {
    ...
    Logger::log(
        "  run              - build and execute the current snowball project");
    Logger::log("  test             - execute tests");
    Logger::log(
        "  init             - create a new snowball project (in current "
        "directory)");
