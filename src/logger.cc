...
void
Logger::rlog(std::string message) {
    printf("%s", message.c_str());
    fflush(stdout);
}
void
Logger::elog(std::string message) {
    fprintf(stderr, "%s\n", message.c_str());
    fflush(stderr);
}
...
void
Logger::help(std::string message) {
    printf("%shelp%s: %s\n", BGRN, RESET, message.c_str());
}
void
...
void
Logger::verbose(std::string message) {
    printf("%sverbose%s: %s\n", BMAG, RESET, message.c_str());
}
void
Logger::success(std::string message) {
    printf("%ssuccess%s: %s\n", BGRN, RESET, message.c_str());
}

void
Logger::message(std::string topic, std::string message) {
...
void
Logger::compiling(std::string message) {
    printf("\33[2K\r%s      Compiling%s %s", BGRN, RESET, message.c_str());
    fflush(stdout);
};

std::string
Logger::progress(float progress) {
