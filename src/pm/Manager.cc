...
int Manager::install(Package p_package) {
  ...
  if (downloadUrl.empty()) {
    throw SNError(Error::PM_ERROR, FMT("Failed to install package '%s'. No download_url found in package info",
                                       p_package.name.c_str()));
  }
  ...
  if (versions.is_array()) {
    ...
    if (!found) {
      throw SNError(Error::PM_ERROR, FMT("Failed to install package '%s'. Version '%s' not found in package info",
                                         p_package.name.c_str(), version.c_str()));
    }
    commitId = version;
  }
  ...
  if (!fs::exists(packageConfig)) {
    throw SNError(Error::PM_ERROR, FMT("Failed to install package '%s'. No sn.toml found in package",
                                       p_package.name.c_str()));
  }
...
nlohmann::json Manager::getPackageInfoFromRepo(std::string p_repo) {
  ...
  if (curl) {
    ...
    if (curlCode != CURLE_OK) {
      throw SNError(Error::PM_ERROR, FMT("Failed to get package info from registry. CURL code %s",
