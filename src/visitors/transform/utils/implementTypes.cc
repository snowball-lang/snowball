...
void Transformer::implementTypes(types::DefinedType* ty, std::vector<Expression::TypeRef*> types, std::vector<Statement::FunctionDef*>& functions) {
    ...
    for (auto type : types) {
        ...
        if (auto interface = utils::cast<types::InterfaceType>(ty)) {
            ...
            E<TYPE_ERROR>(type, FMT("Type '%s' is not an interface!", type->getPrettyName().c_str()), {
            ...
            if (field->kind == types::InterfaceType::Member::Kind::FIELD) {
                ...
                if (classField == typeFields.end()) {
                    if (ast->getValue() == nullptr) {
                        E<TYPE_ERROR>(ty, FMT("Field '%s' is not implemented!", name.c_str()), {
                            .info = "Lacks of implementation!",
                            .note = "All fields of an interface must be implemented!",
                            .help = "Maybe you meant to use 'extends' instead of 'implements'?",
                            .tail = EI<>(ast, "", {
                                .info = "Field declared here!",
                                .note = FMT("Field '%s' declared here without a default\nimplementation values!", name.c_str()),
                            })
                        });
                    }
                    
                    ty->addField(new types::DefinedType::ClassField(name, type, ast->getPrivacy(), ast->getValue(), ast->isMutable()));
                } else {
                    if (!(*classField)->type->is(type)) {
                        E<TYPE_ERROR>(ast, FMT("Field's (%s) type does not match the interface's type!", name.c_str()), {
                            .info = "Type mismatch!",
                            .note = FMT("Field '%s' declared here with a different type!", name.c_str()),
                            .help = FMT("Here it's declared as '%s' but the interface declares it as '%s'!",
                        ...
                        E<TYPE_ERROR>(ast, FMT("Field's (%s) mutability does not match the interface's mutability!", name.c_str()), {
                        ...
                        E<TYPE_ERROR>(ast, FMT("Field's (%s) privacy does not match the interface's privacy!", name.c_str()), {
