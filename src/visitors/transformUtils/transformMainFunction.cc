...
void Transformer::transformMainFunction(ptr<Statement::FunctionDef> p_node) {
    ...
    if (p_node->getArgs().size() > 0) {
        ...
        E<SYNTAX_ERROR>(p_node,
                        "Program entry point can't have and external flag!");
    ...
    if (!returnType->is(ctx->getInt32Type())) {
        E<SYNTAX_ERROR>(rawRetTy,
                        "Program entry's return type must be of type 'i32'!");
    }
