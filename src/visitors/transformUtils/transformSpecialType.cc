...
#define ASSERT_GENERICS(n, t) \
    ...
    if (generics.size() != n) { E<TYPE_ERROR>(ty->getDBGInfo(), FMT("Expected '%i' generics for '%s' but got '%i' instead", n, t.c_str(), generics.size())); }
...
std::shared_ptr<types::Type> Transformer::transformSpecialType(Expression::TypeRef *ty) {
    ...
    STYPE_INSTANCE(FUNCTION_RETURN_STYPE) {
        ...
        E<TYPE_ERROR>(ty, FMT("Type '%i' expected first generic parameter to be a function but it found type '%s'.", FUNCTION_RETURN_STYPE.c_str(), type->getPrettyName().c_str()));
    }
