...
Transformer::getFromIndex(DBGSourceInfo *dbgInfo, Expression::Index *index,
    ...
    auto getFromType = [&](std::shared_ptr<types::Type> type,
        ...
        if (auto x = utils::dyn_cast<types::DefinedType>(type)) {
            ...
            if (!isStatic) {
                ...
                auto fieldValue = std::find_if(fields.begin(), fields.end(),
    ...
    if (auto baseIdentifier = utils::cast<Expression::Identifier>(base)) {
        ...
        if (v && (!isStatic)) {
            ...
            E<VARIABLE_ERROR>(dbgInfo, FMT("Identifier '%s' not found!",
                                           baseIdentifier->getIdentifier().c_str()));
        }
        ...
        E<SYNTAX_ERROR>(dbgInfo, "Static acces/method call can only be used with "
                                 "indentifiers!");
    }

    return {
        {std::nullopt, std::nullopt, std::nullopt, std::nullopt, std::nullopt, false},
        std::nullopt};
}
} // namespace Syntax
} // namespace snowball