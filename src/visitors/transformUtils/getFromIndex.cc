...
Transformer::getFromIndex(DBGSourceInfo *dbgInfo, Expression::Index *index,
    ...
    if (auto baseIdentifier = utils::cast<Expression::Identifier>(base)) {
        auto g =
            utils::cast<Syntax::Expression::GenericIdentifier>(baseIdentifier);
        auto generics = (g != nullptr)
                            ? g->getGenerics()
                            : std::vector<Syntax::Expression::TypeRef *>{};

        auto [val, type, funcs, overloads, mod] = getFromIdentifier(
            dbgInfo, baseIdentifier->getIdentifier(), generics);
