...
SN_TRANSFORMER_VISIT(Expression::PseudoVariable) {
    ...
    if (pseudo == "FN") {
        ...
        if (auto x = ctx->getCurrentFunction()) {
            ...
            E<PSEUDO_ERROR>(p_node, "Can't use @MANGLED_FN pseudo variable outside a function!");
        }
    } else if (pseudo == "MOD") {
    ...
    // We add " to both sides because the generator removes them and we prevent the values
    //  from being lost.
    auto val = new Syntax::Expression::ConstantValue(Expression::ConstantValue::ConstantType::String, "\"" + stringValue + "\"");
    val->accept(this);
}
