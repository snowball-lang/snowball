...
static std::optional<std::string> did_you_mean(
        const std::string& input, const std::vector<std::string>& possibilities,
        uint64_t max_distance = 5
) {
  ...
  for (const auto& possibility : possibilities) {
    ...
    if (input_length == 0 || possibility_length == 0) {
      ...
      for (uint64_t i = 0; i <= possibility_length; i++) { previous[i] = i; }
      for (uint64_t i = 0; i < input_length; i++) {
        current[0] = i + 1;
        for (uint64_t j = 0; j < possibility_length; j++) {
          uint64_t cost = (input[i] == possibility[j]) ? 0 : 1;
          current[j + 1] = std::min(
                  std::min(current[j] + 1, previous[j + 1] + 1), previous[j] + cost
          );
        }
        for (uint64_t j = 0; j <= possibility_length; j++) { previous[j] = current[j]; }
      }
