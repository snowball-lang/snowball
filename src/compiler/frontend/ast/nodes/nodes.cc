...
void AttributedNode::assert_is_var() const {
  sn_assert(!is_abstract || !is_final || !is_override || !is_inline 
    || !is_virtual || !is_operator || !is_test, "Other attributes are not allowed with 'var'");
}

void AttributedNode::assert_is_type() const {
  sn_assert(!is_const || !is_mut || !is_static || !is_override 
    || !is_virtual || !is_inline || !is_operator || !is_test, "Other attributes are not allowed with 'type'");
