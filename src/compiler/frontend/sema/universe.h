...
public:
  ...
  void add_item(const std::string& name, const Item& item) {
...
public:
  ...
  void add_type(const NamespacePath& path, ast::types::Type* type) {
    types[path] = type;
  }
  ...
  std::optional<ast::types::Type*> get_constraint(size_t index) {
    if (index < constraints.size()) { return constraints[index]; }
    return std::nullopt;
  }
  ...
  std::optional<ast::Stmt*> get_var_id(uint64_t id) {
    ...
    if (it != var_ids.end()) { return std::optional(it->second); }
    return std::nullopt;
  }
