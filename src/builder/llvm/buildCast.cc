...
#define IS_FLOAT(x) (utils::dyn_cast<types::Float32Type>(x) || utils::dyn_cast<types::Float64Type>(x))
...
void LLVMBuilder::visit(ir::Cast* c) {
    ...
    if (IS_INTEGER(vTy) && IS_INTEGER(ty)) {                     // i[n] <-> i[n]
        ...
        assert(utils::dyn_cast<types::ReferenceType>(vTy) || utils::dyn_cast<types::CObjectType>(vTy));
    ...
    } else if (utils::dyn_cast<types::NumericType>(ty) && utils::dyn_cast<types::CObjectType>(vTy)) { // cobj -> i[n]
