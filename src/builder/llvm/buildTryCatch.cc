...
void LLVMBuilder::visit(ir::TryCatch* node) {
    auto ourCaughtResultType = llvm::StructType::get(*context,
    ...
    auto ourExceptionNotThrownState =
        llvm::ConstantInt::get(llvm::Type::getInt8Ty(*context), 0);
    auto ourExceptionThrownState =
        llvm::ConstantInt::get(llvm::Type::getInt8Ty(*context), 1);
    auto ourExceptionCaughtState =
    ...
    auto externalExcBlock =
      llvm::BasicBlock::Create(*context, "catch.external", parentFunc);
    auto unwindResumeBlock =
        llvm::BasicBlock::Create(*context, "catch.unwind", parentFunc);
    ...
    auto unwindExceptionClass = builder->CreateLoad(
        ...
        builder->CreateStructGEP(
            unwindType, builder->CreatePointerCast(unwindException, unwindType->getPointerTo()),
            0));
    ...
    llvm::Value *execVal =
        builder->CreatePointerCast(builder->CreateConstGEP1_64(builder->getInt8Ty(), unwindException,
                                                    (uint64_t)exception_offset()),
    ...
    for (int i = 0; i < info.handlers.size(); i++) {
        ...
        castSwitch->addCase(builder->getInt32((uint64_t)typeIdxLookup(mangledType)),
                                    BBlock);
        ...
        build(block.get());
        if (BBlock->getTerminator() == nullptr)
            builder->CreateBr(endBlock);
    }
