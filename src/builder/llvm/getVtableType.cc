...
llvm::StructType* LLVMBuilder::getVtableType(types::DefinedType* ty) {
  if (auto foundType = ctx->getVtableTy(ty->getId())) {
    ...
    auto numberOfFunctions =
            llvm::cast<llvm::ArrayType>(llvm::cast<llvm::StructType>(foundType)->elements()[0])->getNumElements() - 2;
    if (numberOfFunctions == ty->getVtableSize()) return foundType;
  }
